...
export default function TabTechnical({ prices }) {
  if (!prices?.length || prices.length < 35) {
    return (
      <div>
        <ExplainBox
          title="데이터 부족"
          body="기술지표 계산에 필요한 데이터가 부족합니다 (최소 35일 필요)."
          type="warning"
        />
      </div>
    )
  ...
  const histColors = hist.map(v =>
    (v ?? 0) >= 0 ? 'rgba(47, 125, 79, 0.55)' : 'rgba(217, 92, 92, 0.55)'
  )
  ...
  const techData = [
    ...
    },
    {
      ...
      y: bbU,
      name: 'BB상단',
      line: { color: '#d95c5c', width: 1.4, dash: 'dot' },
      yaxis: 'y',
    },
    {
      type: 'scatter',
      ...
      y: ma20,
      name: 'BB중심',
      line: { color: '#e38b2c', width: 1.6, dash: 'dot' },
      yaxis: 'y',
    },
    {
      type: 'scatter',
  ...
  if (lastRSI != null) {
    if (lastRSI >= 70) {
      rsiMsg = `현재 RSI: <b>${lastRSI.toFixed(1)}</b> — 과매수 구간. 단기 조정 가능성에 유의하세요.`
      rsiType = 'warning'
    } else if (lastRSI <= 30) {
      rsiMsg = `현재 RSI: <b>${lastRSI.toFixed(1)}</b> — 과매도 구간. 반등 가능성이 있으나 확인이 필요합니다.`
      rsiType = 'info'
    } else {
      rsiMsg = `현재 RSI: <b>${lastRSI.toFixed(1)}</b> — 중립 구간입니다.`
      rsiType = 'good'
    }
  }
  ...
  return (
    <div>
      ...
      <Expander title="⚖️ RSI — 지금 이 주식이 과열됐나요, 침체됐나요?" defaultOpen>
...
function MAExplain({ close, ma20, ma60 }) {
  ...
  return (
    <>
      ...
      <table className="an-tech-guide-table" style={{ marginTop: 12 }}>
        <thead>
          <tr>
            <th>이동평균선</th>
            <th>기간</th>
      ...
      <div className="an-warning-box" style={{ marginTop: 8 }}>
        📌 이동평균선은 <b>후행(lagging) 지표</b>입니다. 이미 주가가 크게 움직인 다음에 신호가 나타나는 경우가 많습니다.
      </div>
    </>
  )
}
...
function RSIExplain({ rsi }) {
  let status = '⚪ RSI 데이터를 불러올 수 없습니다.'
  let detail = ''

  if (rsi != null) {
    if (rsi >= 70) {
      status = `🔴 과매수 구간 (현재 RSI: ${rsi.toFixed(1)})`
      detail = '최근 주가가 단기간에 많이 올랐습니다. 통계적으로 조정이 올 가능성이 높아지는 구간입니다.'
    } else if (rsi <= 30) {
      status = `🔵 과매도 구간 (현재 RSI: ${rsi.toFixed(1)})`
      detail = '최근 주가가 단기간에 많이 떨어졌습니다. 기술적으로는 반등 가능성이 있는 구간입니다.'
    } else if (rsi >= 50) {
      status = `🟡 중립 상단 (현재 RSI: ${rsi.toFixed(1)})`
      detail = '매수세가 다소 우세한 보통 구간입니다.'
    } else {
      status = `🟡 중립 하단 (현재 RSI: ${rsi.toFixed(1)})`
      detail = '매도세가 다소 우세한 보통 구간입니다.'
    }
  }
  ...
  return (
    <>
      ...
      <div className="an-warning-box" style={{ marginTop: 8 }}>
        📌 RSI가 70을 넘었다고 해서 <b>반드시</b> 떨어지는 것이 아닙니다. 강한 상승 추세에서는 70~90을 몇 달씩 유지하는 경우도 흔합니다.
      </div>
    </>
  )
}
    ...
    <>
      ...
      <table className="an-tech-guide-table" style={{ marginTop: 12 }}>
        ...
        <tbody>
          ...
          <tr>
            <td>시그널선</td>
            <td>MACD선의 9일 이동평균</td>
          </tr>
          <tr>
            <td>히스토그램</td>
            <td>MACD선 - 시그널선 (막대가 클수록 추세 강함)</td>
          </tr>
        </tbody>
      </table>

      <div className="an-warning-box" style={{ marginTop: 8 }}>
        📌 MACD도 이동평균에 기반하므로 후행성이 있습니다. MACD 신호만 보고 매수·매도하는 것은 위험합니다.
      </div>
    </>
  )
}
...
function BBExplain({ close, upper, lower }) {
  ...
  if (close != null && upper != null && lower != null && upper > lower) {
    ...
    if (close >= upper * 0.99) {
      status = `🔴 현재가(${fmtPrice(close)})가 상단 밴드(${fmtPrice(upper)}) 근처입니다 — 단기 과열 구간일 수 있습니다.`
    } else if (close <= lower * 1.01) {
      status = `🔵 현재가(${fmtPrice(close)})가 하단 밴드(${fmtPrice(lower)}) 근처입니다 — 단기 침체 구간일 수 있습니다.`
    } else {
      status = `🟡 현재가(${fmtPrice(close)})가 밴드 안 ${pos.toFixed(1)}% 위치에 있습니다 — 정상 범위 내.`
    }
  }
  ...
  return (
    <>
      ...
      <div className="an-warning-box" style={{ marginTop: 8 }}>
        📌 볼린저밴드만으로 매수·매도 시점을 판단하는 것은 위험합니다. 다른 지표와 함께 사용하세요.
      </div>
    </>
  )
}
