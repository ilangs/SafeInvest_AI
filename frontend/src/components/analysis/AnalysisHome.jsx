...
function EduStock() {
  return (
    <div>
      ...
      <p>예를 들어 삼성전자 주식 1주를 사면, 삼성전자라는 회사의 아주 작은 주인이 되는 것입니다.</p>
      ...
      <ul style={{ marginLeft: 20, marginTop: 8, lineHeight: 2 }}>
        <li>코스피: 삼성전자, 현대차 등 대형 우량기업 중심 입니다.</li>
        <li>코스닥: 중소·벤처기업 중심, 변동성이 더 큽니다.</li>
      </ul>
      <div className="an-warning-box" style={{ marginTop: 16 }}>
        <b>⚠️ 중요한 사실</b><br />주식 투자는 원금 손실이 발생할 수 있습니다. 투자 전 반드시 본인의 재정 상황을 점검하세요.
      </div>
    </div>
  )
}
...
function EduFinancial() {
  return (
    <div>
      ...
      <p>병원에서 혈액검사 결과지를 보듯, 투자자는 재무제표로 기업 상태를 확인합니다.</p>
      <p style={{ marginTop: 14, fontWeight: 700, color: '#93c5fd' }}>재무제표 3가지 핵심 문서</p>
      <div className="an-grid-3" style={{ marginTop: 12 }}>
        {[
          { title: '📋 재무상태표', eng: 'Balance Sheet', desc: '기업이 가진 재산(자산), 빚(부채), 자기 돈(자본)을 보여줍니다.', formula: '자산 = 부채 + 자본' },
          { title: '📈 손익계산서', eng: 'Income Statement', desc: '일정 기간 동안 얼마 벌고 얼마 썼는지 보여줍니다.', formula: '매출액 → 영업이익 → 순이익' },
          { title: '💵 현금흐름표', eng: 'Cash Flow', desc: '실제로 현금이 얼마나 들어오고 나갔는지 보여줍니다.', formula: '이익이 있어도 현금이 부족하면 위험합니다.' },
...
function EduIndicators() {
  const items = [
    { name: 'PER (주가수익비율)', eng: 'Price Earnings Ratio', formula: '주가 ÷ 주당순이익(EPS)', desc: '주가가 1년 이익의 몇 배인지 나타냅니다. PER 10이면 지금 주가로 투자금을 회수하는 데 10년 걸린다는 의미입니다.', caution: '낮을수록 이익 대비 저렴한 편이나, 업종마다 기준이 다릅니다.' },
    { name: 'PBR (주가순자산비율)', eng: 'Price Book-value Ratio', formula: '주가 ÷ 주당순자산(BPS)', desc: '주가가 회사 순자산의 몇 배인지 나타냅니다. PBR 1이면 주가 = 장부상 자산가치입니다.', caution: '1 미만이면 장부가보다 싸게 살 수 있다는 의미이나, 이유가 있을 수 있습니다.' },
    { name: 'ROE (자기자본이익률)', eng: 'Return On Equity', formula: '순이익 ÷ 자기자본 × 100', desc: '주주가 맡긴 돈으로 얼마나 이익을 냈는지 보여줍니다. ROE 15%면 100원을 맡겼을 때 15원을 벌었다는 뜻입니다.', caution: '높을수록 경영 효율이 좋지만, 부채를 많이 써서 ROE가 높은 경우도 있습니다.' },
    { name: 'EPS (주당순이익)', eng: '(Earnings Per Share)', formula: '순이익 ÷ 발행주식수', desc: '주식 1주당 얼마의 이익을 냈는지 보여줍니다.', caution: 'EPS가 매년 증가하는 기업은 꾸준히 이익이 늘고 있다는 신호입니다.' },
    { name: '부채비율', eng: 'Debt Ratio', formula: '부채 ÷ 자본 × 100', desc: '기업이 자기 돈 대비 얼마나 빚을 지고 있는지 나타냅니다.', caution: '일반적으로 200% 이하가 안전하다고 보나 업종마다 다릅니다.' },
    { name: '배당 수익율', eng: 'Dividend Yield', formula: '주당배당금 ÷ 주가 × 100', desc: '주가 대비 배당금 비율입니다. 3%이면 100만원 투자 시 연 3만원의 배당을 받는다는 의미입니다.', caution: '높은 배당수익률이 항상 좋은 것은 아닙니다. 주가가 하락해서 비율이 높아지는 경우도 있습니다.' },
  ]
  return (
    <div>
...
function EduRisks() {
  const items = [
    ...
    { icon: '🔴', title: '거래정지·관리종목', desc: '거래소가 투자자 보호를 위해 지정한 종목입니다.', caution: '관리종목은 상장폐지 가능성이 높습니다. 투자 시 각별한 주의가 필요합니다.' },
  ]
  return (
    <div>
      ...
      <div className="an-warning-box">
        ...
        경고 표시는 공개 공시 데이터를 기반으로 한 사실 정보입니다<br />
...
function EduChecklist() {
  const items = [
    { q: '💰 여유 자금인가요?', a: '생활비, 비상금을 제외한 여유 자금으로만 투자하세요. 필요한 돈으로 투자하면 손실 시 큰 문제가 생깁니다.' },
    { q: '🎯 왜 이 종목에 투자하려 하나요?', a: '누군가의 추천, 커뮤니티 글, SNS만 보고 투자하는 것은 위험합니다. 직접 재무 데이터를 확인하세요.' },
    { q: '📅 얼마나 보유할 계획인가요?', a: '단기 시세 차익을 노리는 투자는 손실 위험이 큽니다. 장기 투자 관점에서 접근하는 것이 일반적으로 안전합니다.' },
    { q: '📉 손실이 나면 어떻게 할 건가요?', a: '미리 손절 기준을 정해두세요.손실이 날 때 감정적 판단을 하면 손실이 더 커질 수 있습니다.' },
    { q: '🏢 이 기업의 사업을 이해하나요?', a: '어떻게 돈을 버는지 모르는 기업에는 투자하지 않는 것이 좋습니다.' },
    { q: '📊 최근 재무 데이터를 직접 확인했나요?', a: '이 앱의 종목 검색 기능으로 매출, 영업이익, 부채비율 등 실제 숫자를 직접 확인하세요.' },
  ]
  return (
    <div>
