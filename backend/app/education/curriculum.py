...
async def list_all_paths():
    ...
    for pid in DISPLAY_ORDER:
        ...
        if not path:
            continue
        result.append(_path_to_card_dict(path, _hydrate_learning_path(path)))
...
def _path_to_card_dict(path: dict, stats: Optional[dict] = None) -> dict:
    stats = stats or {}
    """학습경로 정의 → 카드용 딕셔너리"""
    return {
        "id": path["id"],
        ...
        "learnable_contents": stats.get("learnable_contents", path.get("total_contents", 0)),
        "playable_contents": stats.get("playable_contents", 0),
        "weeks": [
            {
...
async def get_curriculum_detail(path_id: str):
    ...
    from .curriculum_matcher import match_contents_for_path
    hydrated_weeks = match_contents_for_path(
        path,
        contents_by_topic,
        all_contents_by_slno,
    )
    stats = _hydrate_learning_path(path)
    ...
    return {
        ...
        "weekly_hours": path["weekly_hours"],
        "total_contents": path["total_contents"],
        "learnable_contents": stats["learnable_contents"],
        "playable_contents": stats["playable_contents"],
...
# ============================================================
# 헬퍼: 학습경로별 콘텐츠 통계 (카드/상세 공용)
# ============================================================
def _hydrate_learning_path(path: dict) -> dict:
    """
    학습경로에 매칭되는 실제 콘텐츠 통계 계산.

    반환:
      - learnable_contents: 매칭된 전체 콘텐츠 수 (영상+문서 등)
      - playable_contents:  바로 재생/열람 가능한 URL이 있는 콘텐츠 수
    """
    try:
        from .data.mock_data import SAMPLE_CONTENTS
        from .curriculum_matcher import match_contents_for_path
    except Exception:
        # import 실패 시 안전한 기본값 (path 정의값 사용)
        return {
            "learnable_contents": path.get("total_contents", 0),
            "playable_contents": 0,
        }

    # 주제별 / slno별 인덱스 구성
    contents_by_topic = {tc: list(cs) for tc, cs in SAMPLE_CONTENTS.items()}
    all_by_slno = {
        c["contentsSlno"]: c
        for cs in SAMPLE_CONTENTS.values()
        for c in cs
    }

    hydrated_weeks = match_contents_for_path(path, contents_by_topic, all_by_slno)

    learnable = 0
    playable = 0
    for w in hydrated_weeks:
    ...
    return {
        "learnable_contents": learnable,
        "playable_contents": playable,
    }
